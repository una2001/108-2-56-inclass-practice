...
重設levels排序
```{r}
covid19$年齡層 <- factor(covid19$年齡層, levels=names(startingAges))
levels(covid19$年齡層)
```
...
```{r}
str(garbageRoutes$linename)
levels(factor(garbageRoutes$linename , ordered=T))
garbageRoutes$route <- 
  str_extract(garbageRoutes$linename, 
            "[A-Z][0-9][0-9]路線|[A-Z]路線|金山[0-9]路線")
garbageRoutes$route <- factor(garbageRoutes$route)
...
levels(garbageRoutes$route) <- 
  names(sort(table(garbageRoutes$route)))
garbageRoutes$route <- factor(garbageRoutes$route, 
                              levels = new_level)
```
